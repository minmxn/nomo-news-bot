...
- **Groq 429s & error logging.** Groq's **free tier rate-limits** (HTTP 429) under bursts — e.g. the reader firing `filterRelevantNews` + `generateSummaries` back-to-back. `groq.js` wraps every call in `withRetry`, which retries **only** on 429, waiting the `Retry-After` the API returns (capped at 15s; exponential otherwise), up to 3 tries — so brief bursts recover instead of dropping to fallbacks. Every other error fails fast into the existing fallback. `groqErr()` formats failures for logs (HTTP status + response-body snippet, or message/name) so a blank `e.message` never hides the cause; `groqJSON` throws `Groq returned non-JSON: "<snippet>"` on a parse failure for the same reason.
- **API budget.** Scheduled posts use ~7 NewsAPI calls/day (1 each: 8am briefing, 9am poll, 10am MCQ, 6pm evening carousel, and the 12pm/3pm/8pm readers) to stay well under the 100/day free-tier cap. The combined query (`fetchCombinedNews`) replaced 3 separate category fetches. Quota is tracked in `quota.js` (in-memory, resets at SGT midnight).
