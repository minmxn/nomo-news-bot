...
- 🔴 Hard: genuinely difficult — it should stump most non-experts and make even a finance professional stop and think, while staying verifiably correct. The difficulty must come from DEPTH, not ambiguity:
  • Test a second-order / knock-on effect, a precise mechanism, or the application of a concept to a specific scenario — something that needs real understanding, NOT recall of a definition. (e.g. "the Fed signals higher-for-longer rates — what happens to the price of a long-duration bond versus a short-duration one, and why?" — deeper than "what is a bond".)
  • Make the distractors SOPHISTICATED: plausible traps an intermediate learner would fall for — commonly confused pairs (nominal vs real, duration vs maturity, cause vs correlation, stock vs flow, revenue vs profit), or a half-right statement. The challenge is spotting the truly correct one, not guessing.
  • BUT every wrong option must still be DEFINITIVELY wrong to someone who genuinely understands — never "also arguably right". Difficulty comes from how hard the answer is to SEE, never from debate over which answer is correct.
  • The correct answer must be the TEXTBOOK-correct, standard one a professional would confirm with zero debate — not a contrarian "well, actually" take.
  • Test a SINGLE clear mechanism; do NOT string several events into a speculative "given X, Y and Z…" chain.
  • Before finalising, sanity-check the correct answer points in the right DIRECTION (finance relationships are easy to state backwards). If you are not 100% sure it is textbook-correct, pick a different hard-but-solid question you ARE sure about.
  • PREFER reasoning over arithmetic: no multi-step numerical problems (forward pricing, discounting, bond math). Avoid currency/FX-denomination questions ("how a weaker/stronger dollar affects gold / Bitcoin / commodity X") — those are notoriously easy to state backwards.
...
// Throws on failure so the caller (the reader carousel) can fall back to the
// article description.
